...
def append_feedback_to_sheet(row: dict):
    """
    row: dict ordonné -> {"timestamp": "...", "rating": 5, "message": "...", "email": "...", ...}
    Écrit une ligne dans la Google Sheet configurée dans les secrets.
    """
    # Portée d’accès Sheets (et Drive si on ouvre par nom)
    scopes = [
    ...
    # Ouvre par nom (défini dans les Secrets) ; si tu préfères l’ID, utilise open_by_key
    sheet_name = st.secrets["SHEETS"]["name"]
    sh = gc.open(sheet_name)
    ws = sh.sheet1  # ou sh.worksheet("Feedback") si tu as nommé l’onglet

    # Si c'est la première fois, tu peux créer l'en-tête :
    try:
...
st.sidebar.markdown(
    ...
    **Author**
    ...
    <a href="https://www.linkedin.com/in/lorisdurbano/" target="_blank" style="text-decoration: none; color: #0E100A;">
    ...
    if sent:
        if not message.strip():
            ...
            try:
                ...
                # (Optionnel) Fallback local si Sheets indispo
                fb_dir = Path(__file__).parent / "data"
                fb_dir.mkdir(parents=True, exist_ok=True)
                fb_path = fb_dir / "contact_messages.csv"
                header_needed = not fb_path.exists()
                pd.DataFrame([row]).to_csv(
                    fb_path, mode="a", index=False, header=header_needed, encoding="utf-8"
                )
                st.sidebar.warning(
                    f"Sent locally (Sheets error). CSV saved. Error: {e}"
                )
                st.session_state.show_contact = False
